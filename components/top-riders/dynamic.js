...
class TopRiders extends Component {
	...
	async componentDidMount() {
		const leaderboardResponse = await fetch(
			`https://dot-scraper.canthappenashidden.xyz/api/pages/?access_token=${process.env.SCRAPEY_API_KEY}&filter[order]=timestamp%20DESC&filter[where][url]=https://trackleaders.com/${this.props.race.fields.trackleadersRaceId}&filter[limit]=1`
		);
		...
		const leaderboard = sortedLeaders.map((item, index) => {
			return {
				...
				fields: {
					name: item.name,
					distance:
						item.kilometre !== null ? parseFloat(item.kilometre).toFixed(0) : 0
				}
			};
		});
				...
				<TopRidersWrap fl w_100 pr3 pr0_ns mb4>
					...
					<Div measure_narrow>
						{this.state.leaderboard.length ? (
							this.state.leaderboard.map((rider, index) => (
								<Rider
									numbered={false}
									position={index + 1}
									key={rider.sys.id}
