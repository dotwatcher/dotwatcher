...
const HomepagePrimary = ({ block, index, count }) => {
	...
	if (block.race) {
		const isRaceLive = moment().isBetween(
			moment(block.race.fields.raceDate),
			moment(block.race.fields.raceEndDate)
		);

		const todayIsBeforeStartDate = isBefore(
			new Date(),
			block.race.fields.raceDate
		);

		const showBeforeLive =
			block.race.fields.liveBeforeStartDate && todayIsBeforeStartDate;

		const Title =
			isRaceLive || showBeforeLive ? (
				<H2Live
					f2
					f1_ns
					fw6
				...
				<H2 f2 f1_ns fw6 lh_title ma0 bb bw2 b__white pb2 ml4_ns near_white mb3>
					{widont(block.heading)}
				</H2>
			);
		...
		const noun = block.race.fields.slug.includes("paris-brest")
			? "audax"
			: "race";
								...
								<WordsWrap>
									...
									<P f4 f3_ns pt3>
										<Span link underline near_white hover_white>
											{moment(block.race.fields.raceEndDate).isBefore()
