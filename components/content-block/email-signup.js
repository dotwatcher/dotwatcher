	...
	if (block.heading) {
				...
				<Div w_90 w_60_l center pv4 ph3 ph4_ns bg_white_20 white className="cf">
					<Header>
						...
						<Div measure lh_copy f5 f4_ns>
							<ReactMarkdown measure source={block.words} />
						</Div>
					</Header>
					<MailchimpSubscribe
						url={mailchimpURL}
		...
		return (
			<Div mb4 mb5_l pa3 white relative className="bg-gradient-blue cf">
				...
				<P lh-copy ma0>
					For all the latest updates from the bikepacking webosphere and beyond,
					sign up to DotWatcher Digest.
				</P>
				<MailchimpSubscribe
					url={mailchimpURL}
					render={({ subscribe, status, message }) => (
