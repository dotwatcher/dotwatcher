...
${tachyons}`;
const Date = styled.span`${tachyons}`;
    ...
    <React.Fragment>
      {
        races.map(race => {
          const link = {
            href: race.data.calendarOnly ? race.data.website : `/race?slug=${race.data.slug}`,
            as: race.data.calendarOnly ? null : `/race/${race.data.slug}`,
            target: race.data.calendarOnly ? '_blank' : '_self'
          }
          return (
            <CalendarDetails key={race.sys.id} ma0 f6 pv2 b__light_gray className="cf">
              <Link href={link.href} as={link.as} passHref>
                <A link near_black hover_blue underline_hover target={link.target}>
                  {
                    race.data.title
                  }
