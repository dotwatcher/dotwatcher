...
export const WithResults = Page => {
	...
	WithResults.getInitialProps = async ctx => {
		...
		if (year && race) {
			...
			const allResultsResponse = ctx.query.search
				? await fetch(
						apiUrl(`api/search/race-by-name?name=${ctx.query.search}`)
				  )
				: await fetch(apiUrl(`/api/all-races`, ctx.req));

			const allRaces = await allResultsResponse.json();

			const allRidersResponse = ctx.query.search
				? await fetch(apiUrl(`api/search/rider-by-name?name=${value}`).json())
				: [];

			const allRiders = await fetch(
