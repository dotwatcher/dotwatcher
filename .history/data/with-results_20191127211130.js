...
export const WithResults = Page => {
	...
	WithResults.getInitialProps = async ctx => {
		...
		if (year && race) {
			...
			const allResultsResponse = search
				? await fetch(apiUrl(`api/search/race-by-name?name=${search}`))
				: await fetch(apiUrl(`/api/all-races`, ctx.req));

			const allRaces = await allResultsResponse.json();

			const allRiders = search
				? await fetch(apiUrl(`api/search/rider-by-name?name=${search}`).json())
				: [];
