...
class App extends Component {
	...
	async handleSearchUpdate(value) {
		...
		try {
			...
			// remove duplicate occurances of same race name
			const races = res.data.results.reduce((acc, curr) => {
				if (!acc.some(obj => obj.racename === curr.racename)) {
					acc.push({
						...curr,
						events: acc
							.map(race => (race.racename === curr.racename ? race.year : ""))
							.filter(year => year)
					});
					acc.push(curr);
				}
