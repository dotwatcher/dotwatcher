...
class App extends Component {
	...
	async handleQueryUpdate(value) {
		// toLower is performed on the SQL query. Able to lower case race name column and search query at once
		try {
			const races = await fetch(apiUrl(`api/search/race-by-name?name=${vale}`));
		...
		return (
			<Page>
				...
				{this.props.results.length >= 1 ? (
					...
					<ResultsIndex allRaces={this.props.allRaces} />
				)}
				<Footer />
			</Page>
		);
